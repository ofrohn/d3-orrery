...
THREEx.Planets.create = function(body, skipextras) {
  ...
  var m = new THREE.Matrix4(),
      ...
      alpha = THREE.Math.degToRad(p.rotation[0]),
      beta = THREE.Math.degToRad(90-p.rotation[1]),
      gamma = THREE.Math.degToRad(10);

  m1.makeRotationY( alpha );
  m2.makeRotationX( beta );
  m2.makeRotationZ( gamma );
  m.multiplyMatrices( m1, m2 );
  mesh.rotation.setFromRotationMatrix(m, "YXZ");
  
  //mesh.rotation.set(THREE.Math.degToRad(45), 0, THREE.Math.degToRad(-45));
  //mesh.rotation.set(THREE.Math.degToRad(p.rotation[0]), THREE.Math.degToRad(90-p.rotation[1]), 0, "YXZ" );  
  //mesh.rotateY(THREE.Math.degToRad(p.rotation[0]));    
  //mesh.rotateX(THREE.Math.degToRad(90-p.rotation[1]));  
  return mesh;
}
